...
function RSYNC_LOCAL_TO_REMOTE
{
    ...
    E_SSH_OPTIONS=""
    if [ -n "$BACKUP_TGTPORT" ]; then        
        E_SSH_OPTIONS="-e \"ssh -p $BACKUP_TGTPORT\""
    fi
    _SSH_USER=""
    if [ -n "$BACKUP_TGTUSER" ]; then
        _SSH_USER="$BACKUP_TGTUSER@"
    fi

    BACKUP_COMMAND="rsync -a  $E_SSH_OPTIONS $BACKUP_OPTIONS $EXCLUDE_OPTIONS $BACKUP_SRCPATH ${_SSH_USER}$BACKUP_TGTHOST:$BACKUP_TGTPATH"
